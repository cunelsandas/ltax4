        ...
        <div class="animated fadeIn">
            <div class="row">
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                    <div class="card">
                        <div class="card-header" style="text-decoration: underline"><h4>ตรวจสอบภาษีคอนโดมิเนียม</h4></div>
                        <h4 style="margin: 5% auto auto auto">  ยินดีต้อนรับคุณ<font color="#1e90ff">{{ $ownerData->fname }} {{ $ownerData->lname }} (พ.ศ.{{ $getData->annual }})</font></h4>
                        <div class="card-body">
                            ...
                            <br>
                            <td>
                                <a href="{{ url('condo/pds4/' . $getCondoFromPDS4->id) }}"
                                   class="btn btn-block btn-primary">ดู ภ.ด.ส. 4 รายบุคคล</a>
...
{{--                                <button class="tablinks" onclick="openCity(event, 'builds')" id="defaultOpen">โรงเรือนสิ่งปลูกสร้าง</button>--}}
                                ...
                                <button class="tablinks" onclick="openCity(event, 'condo')" id="defaultOpen">คอนโดมิเนียม</button>
                            </div>
...
{{--                            </div>--}}
                        ...
                        <div id="condo" class="tabcontent" style="height: 400px;overflow-x: scroll">
                            @php
                                $ib = 1;
                            @endphp
                            ...
                            <table width="100%">
                                ...
                                <tr style="color: #0080ff">
                                    <td>{{$ib++}}</td>
                                    <td>{{$getCondoFromPDS4->condo_name}}</td>
                                    <td>{{$getCondoFromPDS4->condo_no}}</td>
                                    <td>{{$getCondoData->floor}}</td>
                                    <td>{{$getCondoData->total_area}}</td>                                    {{--                                        <td>{{$getData->bwidth}}X{{$getData->blength}}={{$getData->result_wl}}</td>--}}
                                    <td>@if($getCondoFromPDS4->used_type == 2) 2-อยู่อาศัย @elseif($getCondoFromPDS4->used_type == 3) 3-อื่นๆ @elseif($getCondoFromPDS4->used_type == 4) 4-รกร้างว่างเปล่าหรือไม่ทำประโยชน์ @else ไม่มีข้อมูล  @endif</td>

                                    {{--                                        <td>{{number_format($getData->meanprice_wl),2}} บาท</td>--}}
                                    <td><font style="color: red">{{number_format($getCondoFromPDS4->total_tax,2)}}</font> บาท</td>
                                    {{--                                        <td><font style="color: red">{{number_format($getData->result_final),2}}</font> บาท</td>--}}
                                    <td>{{$getCondoFromPDS4->remark}}</td>
...
{{--                                <p style="font-weight: bold"> ภาษีที่ดิน : {{number_format($sum_tax_lu),2}} บาท </p>--}}
                                                                ...
                                                                <p style="font-weight: bold"> ภาษีคอนโดมิเนียม : {{number_format($getCondoFromPDS4->total_tax,2)}} บาท </p>
                                ...
                                <p style="font-weight: bold"> รวมภาษี : <font style="font-size: 2rem;color: red">{{number_format($getCondoFromPDS4->total_tax,2)}}  บาท </font> </p>
                        ...
                        <div class="col-6" style="text-align: center; margin: 0 auto">
                            <td>
                                <a href="{{ url('/paytaxcondo/' . $getCondoFromPDS4->id) }}" class="btn btn-block btn-success">ชำระภาษี</a>
                                {{--                                <a href="{{ url('/buildings/' . $input->owner_id . '/edit') }}" class="btn btn-block btn-success" >บันทึกข้อมูลโรงเรือน/สิ่งปลูกสร้าง</a>--}}
