            ...
            <div class="row-cols-6">
                <div class="col-sm-12 col-md-12 col-lg-12 col-xl-12">
                    <div class="card">
                        ...
                        <h4 style="margin: 5% auto auto auto"> ชื่อผู้ชำระภาษี <font
                                    color="#1e90ff">{{ $getOwnerData->fname }} {{ $getOwnerData->lname }}</font></h4>
                        <div class="card-body">
                            <tr class="row-cols-6" style="float: left">
                                <td>
                                    {{--                                    <strong>Land_ID: </strong> {{ $getData->land_id }}<br>--}}
                                    <strong>เจ้าของทรัพย์สิน: </strong> {{$getOwnerData->prefix}}{{ $getOwnerData->fname }} {{ $getOwnerData->lname }}
                                    {{--                                        {{ $input->first_name }}--}}
                                    {{--                                        {{ $input->last_name }}--}}

                                    <strong>รหัสบัตรประชาชน: </strong>{{$getOwnerData->pop_id}}

                                </td>
                                <br>
                                <td>
                                    <strong>ที่อยู่: </strong> {{ $getOwnerData->address }}
                                    <strong>หมู่: </strong> {{ $getOwnerData->moo }}
                                    <strong>ซอย: </strong> {{ $getOwnerData->soi }}
                                    <strong>ถนน: </strong> {{ $getOwnerData->road }}
                                </td>
                                <br>
                                <td>
                                ...
                                <td>
                                    ...
                                    <strong>โทร: </strong> {{ $getOwnerData->telephone }}
                            ...
                            <div class="tab">
                                ...
                                <button class="tablinks" onclick="openCity(event, 'condos')">ห้องชุด/คอนโดมิเนียม
                                </button>
                            ...
                            <div id="lands" class="tabcontent" style="height: 400px;overflow-x: scroll">
                                ...
                                <table width="100%">
                                    ...
                                    @if(isset($getFromPDS3))
                                        @foreach($getFromPDS3 as $key => $gfpds3)
                                            <tr style="color: #0080ff">
                                                ...
                                                {{--                                        <td><font style="color: red">{{$gldata->notes}}</font> บาท</td>--}}
                                                <td><font style="color: red">
                                                        {{number_format($gfpds3->total_tax,2)}}</font> บาท
                                                </td>
                                                <input type='hidden' id='sample{{ $loop->index }}'
                                                       value='{{ $gfpds3->total_tax }}'>
                                            </tr>
                            ...
                            <div id="builds" class="tabcontent" style="height: 400px;overflow-x: scroll">
                                ...
                                <table width="100%">
                                    ...
                                    @if(isset($getBuildingJoinPDS3))
                                        @foreach($getBuildingJoinPDS3 as $key => $gbdata)
                                            <tr style="color: #0080ff">
                                                <td>{{$ib++}}</td>
                                                <td>{{$gbdata->parcel_code}}</td>
                                                <td>{{$gbdata->bname}} {{$gbdata->address}} {{$gbdata->moo}} {{$gbdata->soi}} {{$gbdata->road}}</td>
                                                <td>{{$gbdata->no_room}}</td>
                                                <td>{{$gbdata->no_floor}}</td>
                                                {{--                                        <td>{{$getData->bwidth}}X{{$getData->blength}}={{$getData->result_wl}}</td>--}}
                                                <td>{{$gbdata->width}}X{{$gbdata->length}}</td>
                                                {{--                                        <td>{{number_format($getData->meanprice_wl),2}} บาท</td>--}}
                                                <td>{{number_format($gbdata->net_b_price),2}} บาท</td>
                                                {{--                                        <td><font style="color: red">{{number_format($getData->result_final),2}}</font> บาท</td>--}}
                                                <td>
                                                    <font style="color: red">{{number_format($gbdata->total_tax,2)}} </font>บาท
                                                </td>
                                                <input type='hidden' id='sample{{ $loop->index }}'
                                                       value='{{ $gbdata->total_tax }}'>
                                            </tr>
                            ...
                            <div id="condos" class="tabcontent" style="height: 400px;overflow-x: scroll">
                                @php
                                    $ic = 1;
                                @endphp
                                ...
                                <table width="100%">
                                    <tr>
                                        <th width="5%">ลำดับ</th>
                                        <th width="5%">ชื่ออาคารชุด</th>
                                        <th width="20%">เลขที่</th>
                                        <th width="5%">ขนาด</th>
                                        <th width="5%">การใช้ประโยชน์</th>
                                        {{--                                        <th  width="10%">ราคาประเมินต่อตร.ม.</th>--}}
                                        <th width="10%">ภาษี</th>
                                        {{--                                        <th  width="10%">ราคารวมที่ดินและสิ่งปลูกสร้างหลังหักค่าเสื่อม</th>--}}
                                        <th width="10%">หมาตเหตุ</th>
                                    </tr>
                                    @if(isset($getCondoPDS4))
                                        @foreach($getCondoPDS4 as $key => $gcdpds4)
                                            <tr style="color: #0080ff">
                                                <td>{{$ic++}}</td>
                                                <td>{{$gcdpds4->condo_name}}</td>
                                                <td>{{$gcdpds4->condo_no}}</td>
                                                <td>{{$gcdpds4->total_area}}</td>
                                                {{--                                        <td>{{$getData->bwidth}}X{{$getData->blength}}={{$getData->result_wl}}</td>--}}
                                                <td>@if($gcdpds4->used_type==2) อยู่อาศัย @endif</td>
                                                {{--                                        <td>{{number_format($getData->meanprice_wl),2}} บาท</td>--}}
                                                <td>{{number_format($gcdpds4->total_tax,2)}} บาท</td>
                                                {{--                                        <td><font style="color: red">{{number_format($getData->result_final),2}}</font> บาท</td>--}}
                                                <td>
                                                    <font style="color: red">{{number_format($gcdpds4->total_tax,2)}} </font>บาท
                                                </td>
                                                <input type='hidden' id='sample{{ $loop->index }}'
                                                       value='{{ $gcdpds4->total_tax }}'>
                                            </tr>
                                            ...
                                            <tr style="color: #0080ff">
                                                ...
                                                <td>
                                                    ...
                                                    @elseif($gsd10->sign_type_id==4)
                                                        ประเภท 3(ข) อักษรไทยบางส่วนหรือทั้งหมดอยู่ใต้
                                                ...
                                                <td>{{$gsd10->no_side}}</td>
                                                <td>{{$gsd10->s_desc}}</td>
                                                <td><font style="color: red">{{number_format($gsd10->tax,2)}}</font> บาท
                                                </td>
                                                <input type='hidden' id='sample{{ $loop->index }}'
                                                       value='{{ $gsd10->tax }}'>
                                            </tr>
                        ...
                        <div style="margin: 0 auto">
                            <td>
                                ...
                                <p style="font-weight: bold;text-align: center"> ภาษีที่ดิน
                                    : {{ number_format($getFromPDS3->sum('total_tax'),2) }} บาท </p>
                                {{--                                @endif--}}
                                {{--                                @if(isset($getBuildData))--}}
                                {{--                                    <p style="font-weight: bold" hidden> ภาษีโรงเรือน : {{$sum_tax_build+=$getBuildData->tax}} บาท </p>--}}
                                <p style="font-weight: bold;text-align: center"> ภาษีโรงเรือน
                                    : {{number_format($getBuildingJoinPDS3->sum('total_tax'),2)}} บาท </p>

                                <p style="font-weight: bold;text-align: center"> ภาษีห้องชุด-คอนโด
                                    : {{number_format($getCondoPDS4->sum('total_tax'),2)}} บาท </p>
                                {{--                                @endif--}}
                                {{--                                @if(isset($getSignData))--}}
                                {{--                                        <p style="font-weight: bold" hidden> ภาษีป้าย : {{$sum_tax_sign+=$gsd->tax}} บาท </p>--}}
                                <p style="font-weight: bold;text-align: center"> ภาษีป้าย
                                    : {{number_format($getSignData10->sum('tax'),2)}} บาท </p>
                            ...
                            <hr style="border: solid 1px #c3c3c3">
                            <p style="font-weight: bold;font-size: 16px;text-align: center">รวมภาษีทั้งหมด <label
                                        style="color: red;font-size: 20px;"> {{number_format($getFromPDS3->sum('total_tax')+$getBuildingJoinPDS3->sum('total_tax')+$getCondoPDS4->sum('total_tax')+$getSignData10->sum('tax'),2)}} </label>
                                บาท</p>
                            <p style="font-weight: bold;text-align: center">
                                ({{ baht_text(($getFromPDS3->sum('total_tax')+$getBuildingJoinPDS3->sum('total_tax')+$getSignData10->sum('tax')))}}
                                )</p>
                            <br>


                            <div style="border: solid 1px black;padding: 10px;width: 600px;">
                                <p style="text-align: center;font-size: 16px;text-decoration: underline">
                                    การยืนยันสถานะจากผู้ชำระภาษี</p>
                                <div class="row">
                                    <div class="col-4">
                                        @isset($getdateland)
                                            @if($getdateland->status == 1)
                                                <p style="color: green">มีการยืนยันชำระภาษีที่ดิน</p>
                                            @else
                                                <p style="color: red">ยังไม่มีการยืนยัน</p>
                                            @endif
                                        @endisset
                                    </div>
                                    <div class="col-4">
                                        @isset($getdatecondo)
                                            @if($getdatebuild->status == 1)
                                                <p style="color: green">มีการยืนยันชำระภาษีคอนโด</p>
                                            @else
                                                <p style="color: red">ยังไม่มีการยืนยัน</p>
                                            @endif
                                        @endisset
                                    ...
                                    <div class="col-4">
                                        @isset($getdatesign)
                                            @if($getdatesign->status == 1)
                                                <p style="color: green">มีการยืนยันชำระภาษีที่ป้าย</p>
                                            @else
                                                <p style="color: red">ยังไม่มีการยืนยัน</p>
                                            @endif
                                        @endisset
                            ...
                            <div style="border: solid 1px black;padding: 10px">
                                <p style="text-align: center;font-size: 16px;text-decoration: underline">
                                    ยืนยันสถานะการชำระภาษีจากฝั่งเจ้าหน้าที่</p>
                                <div class="row">
                                    @isset($getdateland)
                                        @if($getdateland->status == 1 && $getdateland->status_backend ==0)
                                            <div class="col-4">
                                                <form method="post" action="{{url('/confirmbackland')}}"
                                                      enctype="multipart/form-data">
                                                    {{csrf_field()}}
                                                    <div class="form-group">
                                                        <a onclick="return confirm('ยืนยันให้ชำระภาษีที่ดิน?')">
                                                            <input type="submit" align="left" name="confirm"
                                                                   class="btn btn-block btn-success"
                                                                   value="ยืนยันให้ชำระภาษีที่ดิน">
                                                        </a>
                                                    </div>

                                                </form>
                                            </div>
                                        @elseif($getdateland->status == 1 && $getdateland->status_backend ==1)
                                            <div class="col-4">
                                                <div class="form-group">
                                                    <a>
                                                        <input type="button" align="left" name="confirm"
                                    ...
                                    @isset($getdatebuild)
                                        @if($getdatebuild->status == 1 && $getdatebuild->status_backend ==0)
                                            <div class="col-4">
                                                <form method="post" action="{{url('/confirmbackbuild')}}"
                                                      enctype="multipart/form-data">
                                                    {{csrf_field()}}
                                                    <div class="form-group">
                                                        <a onclick="return confirm('ยืนยันให้ชำระภาษีสิ่งปลูกสร้าง?')">
                                                            <input type="submit" align="left" name="confirm"
                                                                   class="btn btn-block btn-success"
                                                                   value="ยืนยันให้ชำระภาษีสิ่งปลูกสร้าง">
                                                        </a>
                                                    </div>

                                                </form>
                                            </div>
                                        @elseif($getdatebuild->status == 1 && $getdatebuild->status_backend ==1)
                                            <div class="col-4">
                                                <div class="form-group">
                                                    <a>
                                                        <input type="button" align="left" name="confirm"
                                    ...
                                    @endisset
                                    @isset($getdatesign)
                                        @if($getdatesign->status == 1 && $getdatesign->status_backend ==0)
                                            <div class="col-4">
                                                <form method="post" action="{{url('/confirmbacksign')}}"
                                                      enctype="multipart/form-data">
                                                    {{csrf_field()}}
                                                    <div class="form-group">
                                                        <a onclick="return confirm('ยืนยันให้ชำระภาษีป้าย?')">
                                                            <input type="submit" align="left" name="confirm"
                                                                   class="btn btn-block btn-success"
                                                                   value="ยืนยันให้ชำระภาษีป้าย">
                                                        </a>
                                                    </div>

                                                </form>
                                            </div>
                                        @elseif($getdatesign->status == 1 && $getdatesign->status_backend ==1)
                                            <div class="col-4">
                                                <div class="form-group">
                                                    <a>
                                                        <input type="button" align="left" name="confirm"
                        ...
                        <div class="row ml-2 mr-2 mt-2 mb-4">
                            <div class="col-4">
                                <p style="font-weight: bold;font-size: 16px;text-align: left">
                                    สลิปธนาคารที่ชำระภาษีที่ดิน</p>
                                @if(isset($getdateland->upload_date))
                                    <div style="border: 2px solid black">
                                        {{--                                                เลขโฉนดที่ดิน {{$getdateland->ln}}<br>--}}
                                        <a style="cursor: zoom-in"
                                           href="../../paytaxland/images/{{$getdateland->img_paytax_land}}?id={{time()}}"
                                           target="_blank">
                                            <img width="100%" id="imgland" style="margin: 0 auto"
                                                 src="../../paytaxland/images/{{$getdateland->img_paytax_land}}?id={{time()}}"></a>
                                        <br>
                            ...
                            <div class="col-4">
                                <p style="font-weight: bold;font-size: 16px;text-align: center">
                                    สลิปธนาคารที่ชำระภาษีสิ่งปลูกสร้าง</p>
                                @if(isset($getdatebuild->upload_date))
                                    @if(file_exists( public_path().'/paytaxbuild/images/'. $getOwnerData->owner_id .'_paytaxbuild_'.$getdatebuild->upload_date.'.png' ))
                                        <a style="cursor: zoom-in"
                                           href="../../paytaxbuild/images/{{$getOwnerData->owner_id}}_paytaxbuild_{{$getdatebuild->upload_date}}.png?id={{time()}}"
                                           target="_blank"> <img width="70%" id="imgbuild" style="margin: 0 auto"
                                                                 src="../../paytaxbuild/images/{{$getOwnerData->owner_id}}_paytaxbuild_{{$getdatebuild->upload_date}}.png?id={{time()}}"></a>
                                    @else
                                        {{--                            <img src="/images/photos/account/default.png" alt="">--}}
                                    @endif
                            ...
                            <div class="col-4">
                                <p style="font-weight: bold;font-size: 16px;text-align: center">
                                    สลิปธนาคารที่ชำระภาษีป้าย</p>
                                @if(isset($getdatesign->upload_date))
                                    @if(file_exists( public_path().'/paytaxsign/images/'. $getOwnerData->owner_id .'_paytaxsign_'.$getdatesign->upload_date.'.png' ))
                                        <a style="cursor: zoom-in"
                                           href="../../paytaxsign/images/{{$getOwnerData->owner_id}}_paytaxsign_{{$getdatesign->upload_date}}.png?id={{time()}}"
                                           target="_blank"><img width="70%" id="imgsign" style="margin: 0 auto"
                                                                src="../../paytaxsign/images/{{$getOwnerData->owner_id}}_paytaxsign_{{$getdatesign->upload_date}}.png?id={{time()}}"></a>
                                    @else
                                        {{--                            <img src="/images/photos/account/default.png" alt="">--}}
                                    @endif
                                @endif
