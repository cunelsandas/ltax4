...
{{--                                    บาท </p>--}}
                                <p style="font-weight: bold"> รวมภาษีที่ดินที่ต้องชำระ : <font
                                            style="font-size: 2rem;color: red">{{number_format($sumtaxlandbuild,2)}}
                                        บาท </font></p>
                                <p style="font-weight: bold">(<?php echo baht_text($sumtaxlandbuild)  ?>)</p>


                                @if(empty($getdate))

                                    <form method="post" action="{{url('/confirmland')}}" enctype="multipart/form-data">
                                        {{csrf_field()}}
                                ...
                                @isset($getdate)
                                    ...
                                    @if($getcon==1 && $getbackcon==1)
                                        ...
                                        curl_close($ch);
                                                    @endphp
                                                @endif
                                                <form method="post" action="{{url('/uploadfileland')}}"
                                                ...
                                                @if(isset($getdate->upload_date))
                                                    @if(file_exists( public_path().'/paytaxland/images/'. $ownerData->owner_id .'_'.$getLandData->lid.'_'.$getLandData->ln.'_paytaxland_'.$getdate->upload_date.'.png' ))
                                                        <img width="50%" style="margin: 0 auto"
                                                             src="../paytaxland/images/{{$ownerData->owner_id}}_{{$getLandData->lid}}_{{$getLandData->ln}}_paytaxland_{{$getdate->upload_date}}.png?id={{time()}}">
                                                    @else
